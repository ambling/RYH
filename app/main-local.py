...
os.system("python $HADOOP_HOME/app/divide/divide.py 0.0001 1")
...
afterPreRender = time.time()
print "Pre-render successful."
print "Time elapse: " + str(afterPreRender - start)
...
'''
command = "cat input | $HADOOP_HOME/app/render/mapper.py " + xml_file + \
		" | $HADOOP_HOME/app/render/reducer.py " + image_width + ' ' + image_height +\
		" > outputfile"
...
print "Redering time elapse: " + str(e1 - s1)
...
os.system("cat outputfile | python $HADOOP_HOME/app/render/tgaFromstring.py "\
		+ image_width + ' ' + image_height + ' ' + job_name + '_bylocal.tga')
